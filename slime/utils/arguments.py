...
def get_slime_extra_args_provider(add_custom_arguments=None):
    def add_slime_arguments(parser):
        ...
        def add_on_policy_distillation_arguments(parser):
            ...
            parser.add_argument(
                "--self-distill-kl-scope",
                type=str,
                choices=["off", "full_vocab", "sampled_token"],
                default="off",
                help=(
                    "Scope for self-distillation divergence. "
                    "'off' disables self-distillation; "
                    "'full_vocab' matches full-distribution KL/alpha-JS; "
                    "'sampled_token' uses only sampled token log-probs."
                ),
            )
            parser.add_argument(
                "--self-distill-teacher-prompt-key",
                type=str,
                default="teacher_prompt",
                help="Metadata key name used by self-distillation data source for teacher prompt text.",
            )
            parser.add_argument(
                "--self-distill-divergence",
                type=str,
                choices=["forward_kl", "reverse_kl", "alpha_js"],
                default="alpha_js",
                help=(
                    "Token-level divergence used by self-distillation custom loss. "
                    "Use alpha_js with --self-distill-alpha for DistilTrainer-equivalent behavior."
                ),
            )
            parser.add_argument(
                "--self-distill-alpha",
                type=float,
                default=0.0,
                help="Mixing coefficient for alpha_js divergence in self-distillation custom loss.",
            )
            parser.add_argument(
                "--self-distill-top-entropy-quantile",
                type=float,
                default=1.0,
                help="Keep only top-quantile entropy tokens in self-distillation custom loss; 1.0 keeps all tokens.",
            )
            parser.add_argument(
                "--self-distill-num-loss-tokens-to-skip",
                type=int,
                default=0,
                help="Skip first N response tokens when computing self-distillation loss.",
            )
            parser.add_argument(
                "--self-distill-importance-correction",
                action=argparse.BooleanOptionalAction,
                default=True,
                help="Apply rollout/train mismatch correction in self-distillation custom loss.",
            )
            parser.add_argument(
                "--self-distill-importance-cap",
                type=float,
                default=2.0,
                help="Upper cap for self-distillation mismatch correction ratio.",
            )
            return parser
...
def slime_validate_args(args):
    ...
    if not 0.0 <= args.self_distill_alpha <= 1.0:
        raise ValueError(
            f"self_distill_alpha must be in [0, 1], got {args.self_distill_alpha}."
        )
    if not 0.0 <= args.self_distill_top_entropy_quantile <= 1.0:
        raise ValueError(
            "self_distill_top_entropy_quantile must be in [0, 1], "
            f"got {args.self_distill_top_entropy_quantile}."
        )
    if args.self_distill_num_loss_tokens_to_skip < 0:
        raise ValueError(
            "self_distill_num_loss_tokens_to_skip must be >= 0, "
            f"got {args.self_distill_num_loss_tokens_to_skip}."
        )
    if args.self_distill_importance_cap <= 0:
        raise ValueError(
            f"self_distill_importance_cap must be > 0, got {args.self_distill_importance_cap}."
        )
    if args.self_distill_kl_scope != "off":
        if args.loss_type != "custom_loss":
            raise ValueError("self-distillation requires --loss-type custom_loss.")
        if args.train_backend == "megatron":
            if args.custom_loss_function_path is None:
                raise ValueError(
                    "Megatron self-distillation requires --custom-loss-function-path."
                )
            if args.self_distill_kl_scope == "full_vocab":
                if not args.use_opd or args.opd_type != "megatron":
                    raise ValueError(
                        "Megatron full_vocab self-distillation requires --use-opd --opd-type megatron "
                        "so teacher full-vocab distributions are available."
                    )
        elif args.train_backend == "fsdp":
            if args.self_distill_kl_scope == "full_vocab" and args.ref_load is None:
                raise ValueError(
                    "FSDP full_vocab self-distillation requires --ref-load (teacher checkpoint path)."
                )
            if args.self_distill_kl_scope == "full_vocab" and not os.path.exists(args.ref_load):
                raise FileNotFoundError(
                    f"ref_load {args.ref_load} does not exist, please check the path."
                )
            if args.self_distill_kl_scope == "sampled_token" and args.ref_load is not None and not os.path.exists(args.ref_load):
                raise FileNotFoundError(
                    f"ref_load {args.ref_load} does not exist, please check the path."
                )
