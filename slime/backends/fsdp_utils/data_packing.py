...
def pack_sequences(
    ...
    returns: list[float],
    teacher_tokens: list[list[int]] | None = None,
    ...
    for indices in partitions:
        ...
        for i in indices:
            ...
            if teacher_tokens is not None:
                seq_teacher_tokens = teacher_tokens[i]
                flat_teacher_tokens.extend(seq_teacher_tokens)
                flat_teacher_positionids.extend(list(range(len(seq_teacher_tokens))))
                teacher_cu_seqlens.append(teacher_cu_seqlens[-1] + len(seq_teacher_tokens))
        ...
        if teacher_tokens is not None:
            packed_batch["teacher_tokens"] = torch.tensor(flat_teacher_tokens, dtype=torch.long)
            packed_batch["teacher_position_ids"] = torch.tensor(flat_teacher_positionids, dtype=torch.int)
            packed_batch["teacher_cu_seqlens"] = torch.tensor(teacher_cu_seqlens, dtype=torch.int32)
...
def unpack_sequences(packed_batch: dict) -> list[dict]:
    ...
    teacher_cu_seqlens = packed_batch.get("teacher_cu_seqlens", None)
        ...
        for key, value in packed_batch.items():
            if key not in instance:
                ...
                # For tensor attributes, we need to slice them appropriately
                    ...
                    elif key in ["teacher_tokens", "teacher_position_ids"]:
                        if teacher_cu_seqlens is None:
                            raise ValueError(f"{key} exists but teacher_cu_seqlens is missing in packed_batch")
                        teacher_start_idx = teacher_cu_seqlens[i].item()
                        teacher_end_idx = teacher_cu_seqlens[i + 1].item()
                        instance[key] = value[teacher_start_idx:teacher_end_idx]
