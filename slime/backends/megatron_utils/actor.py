...
from .loss import compute_advantages_and_returns, get_log_probs_and_entropy, get_log_probs_and_full_log_probs, get_values
...
class MegatronTrainRayActor(TrainRayActor):
    ...
    def _get_rollout_data(self, rollout_data_ref: Box) -> RolloutBatch:
        ...
        for token_key in ["tokens", "teacher_tokens"]:
            if token_key in rollout_data:
                rollout_data[token_key] = [
                    torch.tensor(t, dtype=torch.long, device=torch.cuda.current_device()) for t in rollout_data[token_key]
                ]
        ...
        if self.args.qkv_format == "bshd":
            ...
            if "teacher_total_lengths" in rollout_data and "teacher_tokens" in rollout_data:
                teacher_max_seq_len = max(rollout_data["teacher_total_lengths"])
                teacher_max_seq_len = (teacher_max_seq_len + pad_size - 1) // pad_size * pad_size
                rollout_data["teacher_max_seq_lens"] = [teacher_max_seq_len] * len(rollout_data["teacher_tokens"])

        for key in ["rollout_log_probs", "teacher_log_probs"]:
            if key not in rollout_data:
                continue
            total_lengths_key = "teacher_total_lengths" if key == "teacher_log_probs" else "total_lengths"
            if total_lengths_key not in rollout_data:
                total_lengths_key = "total_lengths"
            max_seq_lens_key = "teacher_max_seq_lens" if key == "teacher_log_probs" else "max_seq_lens"
            rollout_data[key] = [
                torch.tensor(
                    (
                        slice_log_prob_with_cp(
                            ...
                            rollout_data[max_seq_lens_key][i]
                            if self.args.qkv_format == "bshd" and max_seq_lens_key in rollout_data
                            else None,
                        )
                    ),
                ...
                for i, (log_prob, total_length, response_length) in enumerate(
                    zip(
                        rollout_data[key],
                        rollout_data[total_lengths_key],
    ...
    def compute_log_prob(
        ...
        total_lengths_key: str = "total_lengths",
        max_seq_lens_key: str = "max_seq_lens",
        ...
        with timer(f"{store_prefix}log_probs"):
            return forward_only(
                ...
                total_lengths_key=total_lengths_key,
                max_seq_lens_key=max_seq_lens_key,
            )

    def compute_full_log_prob(
        self,
        data_iterator: list[DataIterator],
        num_microbatches: list[int],
        store_prefix: str = "",
        ...
        with timer(f"{store_prefix}full_log_probs"):
            return forward_only(
                get_log_probs_and_full_log_probs,
                self.args,
                self.model,
                data_iterator,
                num_microbatches,
        ...
        with inverse_timer("train_wait"), timer("train"):
            if self.args.compute_advantages_and_returns:
                ...
                if "teacher" in self.weights_backuper.backup_tags:
                    ...
                    teacher_tokens_key = "teacher_tokens" if "teacher_tokens" in rollout_data else "tokens"
                    teacher_total_lengths_key = (
                        "teacher_total_lengths" if "teacher_total_lengths" in rollout_data else "total_lengths"
                    )
                    teacher_max_seq_lens_key = (
                        "teacher_max_seq_lens" if "teacher_max_seq_lens" in rollout_data else "max_seq_lens"
                    )
                    if getattr(self.args, "self_distill_kl_scope", "off") == "full_vocab":
                        rollout_data.update(
                            self.compute_full_log_prob(
                                data_iterator,
                                num_microbatches,
                                store_prefix="teacher_",
                                tokens_key=teacher_tokens_key,
                                total_lengths_key=teacher_total_lengths_key,
                                max_seq_lens_key=teacher_max_seq_lens_key,
                            )
                        )
                    else:
                        rollout_data.update(
