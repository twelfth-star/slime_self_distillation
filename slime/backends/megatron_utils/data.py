...
def get_batch(
    ...
    assert tokens_key in keys, f"Missing token key '{tokens_key}' in batch keys: {keys}"
    ...
    total_lengths = batch[total_lengths_key]
    max_seq_lens = batch.get(max_seq_lens_key, None)
    ...
    if qkv_format == "bshd":
        if max_seq_lens is None:
            raise ValueError(f"Missing max sequence lengths key '{max_seq_lens_key}' for qkv_format=bshd")
    ...
    batch["total_lengths"] = total_lengths
    if max_seq_lens is not None:
        batch["max_seq_lens"] = max_seq_lens
