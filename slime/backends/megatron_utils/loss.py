...
def distributed_log_softmax(logits: torch.Tensor, tp_group) -> torch.Tensor:
    """Compute TP-aware log-softmax on vocab-parallel logits."""
    ...
    return shifted_logits - sum_exp.log()


def get_log_probs_and_full_log_probs(
    logits: torch.Tensor,
    *,
    args: Namespace,
    unconcat_tokens: list[torch.Tensor],
    ...
    """Compute selected log-probs and local vocab log-probs for response tokens.

    This is used by strict self-distillation to provide teacher full-vocab
    distributions (local TP shard) to the training loss.
    """
    assert non_loss_data
    ...
    for logits_chunk, tokens_chunk in get_responses(
        ...
        local_full_log_probs = distributed_log_softmax(logits_chunk, tp_group)
        ...
        full_log_probs_list.append(local_full_log_probs)
        if with_entropy:
            entropy_list.append(compute_entropy_from_logits(logits_chunk.clone(), tp_group))

    res = {
        "log_probs": selected_log_probs_list,
        "full_log_probs": full_log_probs_list,
    }
    if with_entropy:
        res["entropy"] = entropy_list
    return torch.empty((0,), device=logits.device), res
